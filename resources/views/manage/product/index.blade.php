    ...
    <div class="row justify-content-center">
      ...
      <div class="col-md-12">
        ...
        <div class="panel-body container col-md-12" style="text-align: left; ">
            ...
            <a role="link" class="btn btn-primary"  href="/manage/products/create/" >
                ...
                </svg>  &nbsp; new product </a><!--</button> -->

                <button style="margin-bottom: 10px" class="btn btn-danger delete_all" data-url="/manage/products/myProductDeleteAll">Delete All Selected</button>
        </div>
        ...
        <!-- <table class="table table-striped table-sm"> -->
          ...
          <tbody>
            ...
            @foreach ($products as $product)
              <tr id="tr_{{ $product->id }}" role="row">
                <td style="text-align: center; vertical-align: middle; width:36px; ">
                  <input type="checkbox" class="sub_chk" data-id="{{ $product->id }}">
              </td>

                <td class="card-body">{{ $product->name }}</td>
                <td>{{ (\App\Http\Controllers\LanguagesController::get_language_name($product->language_id)) }}</td>
                <td>
                  @if ($product->shw  == 'Y')
                    <button style="background-color: #00913B;" type="button" title="Draft" class="btn btn-success" onclick="window.location.replace('/manage/products/{{ $product->id }}/disable')" class="btn btn-xs btn-primary edit">
                      Published
                  ...
                  @else
                    <button type="button" title="Enable" class="btn btn-outline-danger" onclick="window.location.replace('/manage/products/{{ $product->id }}/enable')" class="btn btn-xs btn-primary edit" >
                ...
                <td class="card-body">{{ $product->updated_at }}</td>

                <td style="text-align=center; width:auto;">
                  <a class="btn btn-xs btn-primary edit" href="/manage/products/{{ $product->id }}/">
                ...
                <td style="text-align: center; width:auto;">
                                        ...
                                        <a href="/manage/products/{{ $product->id }}/edit/" class="btn btn-xs btn-primary edit" id="{{ $product->id }}">
                                ...
                                <td style="text-align: center; width:auto;">
                                      ...
                                      <a onclick="return confirm('Are you sure you want to delete?')" href="/manage/products/{{ $product->id }}/delete" class="btn btn-xs btn-danger delete" id="{{ $product->id }}">
