	...
	<div class="col-md-12">
        <div class="panel panel-default">
          <div class="panel-heading main-color-bg">
            <h3 class="panel-title">{{ __('Menu Details') }}</h3>
          </div>
        </div>
     </div>
  ...
  <div class="row justify-content-center"> -->
    <!-- <div class="col-md-12">--><div class="well">
      <!-- if there are creation errors, they will show here -->
        ...
        <div class="panel panel-default">
          <div class="panel-heading main-color-bg">
            <h4 class="panel-title">{{ __('Menu Items') }}</h4>
          </div>
        </div>
     </div>
     @if( session()->has( 'message' ))
     ...
     <div class="panel-body container col-md-12" style="text-align: left; ">
            ...
            <a role="link" class="btn btn-primary"  href="/manage/menu/{{$menu->id}}/item/create/" >
                ...
                </svg>  &nbsp; Add item </a><!--</button> -->

                <button style="margin-bottom: 10px" class="btn btn-danger delete_all" data-url="menu/{{$menu->id}}/item/mymenuDeleteAll">Delete All Selected</button>
        </div>
      <div class="col-md-12 col-lg-12 col-sm-12">
        ...
        <!-- <table class="table table-striped table-sm"> -->
          ...
          <tbody>
         	...
         	<tr id="tr_{{ $menu->id }}" role="row">
                <td style="text-align: center; vertical-align: middle; width:36px; ">
                  <input type="checkbox" class="sub_chk" data-id="{{ $menu_item->id }}">
              	</td>
         		<td class="card-body">
							{{ $menu_item->title }}
						</td>
				...
				<td>
                  @if ($menu_item->shw  == 'Y')
                    <button style="background-color: #00913B;" type="button" title="Draft" class="btn btn-success" onclick="window.location.replace('/manage/menu/{{$menu->id}}/item/{{ $menu_item->id }}/disable')" class="btn btn-xs btn-primary edit">
                                Published
                  ...
                  @else
                    <button type="button" title="Enable" class="btn btn-outline-danger" onclick="window.location.replace('/manage/menu/{{$menu->id}}/item/{{ $menu_item->id }}/enable')" class="btn btn-xs btn-primary edit">
                ...
                <td style="text-align: center; width:auto;">
                                        ...
                                        <a href="/manage/menu/{{ $menu->id }}/item/{{ $menu_item->id }}/edit" class="btn btn-xs btn-primary edit" id="{{ $menu->id }}">
                                          <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-pencil-square" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                              <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456l-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
                                              <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"/>
                                          </svg>
                                ...
                                <td style="text-align: center; width:auto;">
                                      ...
                                      <a onclick="return confirm('Are you sure you want to delete?')" href="/manage/menu/{{ $menu->id }}/item/{{ $menu_item->id }}/delete" class="btn btn-xs btn-danger delete" id="{{ $menu_item->id }}">
         ...
         <div style="text-align: center;" class="form-group">
              <div style="text-align: center;">
                ...
                <a class="btn btn-primary" href="/manage/menu/{{$menu->id}}">Cancel</a>
              </div>
            </div>
    </form>
   </div>
        ...
        $(document).ready(function () {
                    ...
                    if(check == true) {
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                              window.location.replace("/manage/menu{{$menu->id}}/item/{{$menu_item->id}}/delete");
                                // if (data['success']) {
                                //     // $(".sub_chk:checked").each(function() {
            ...
            $(document).on('confirm', function (e) {
                ...
                $.ajax({
                    ...
                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                    success: function (data) {
                      window.location.replace("/manage/menu{{$menu->id}}/item/{{$menu_item->id}}");
                        // if (data['success']) {
